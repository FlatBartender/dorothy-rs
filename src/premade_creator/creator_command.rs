...
#[description("Gets the configuration for this server if it's already loaded.")]
fn get(_ctx: &mut Context, msg: &Message, _args: Args) -> Result<(), CommandError> {
...
#[command]
#[description("Creates a new configuration. The start and end expressions are cron syntax, but with an additional field on the left for seconds.")]
#[usage("<channel_id> <start exp> <end exp>")]
#[num_args(3)]
#[only_in("guilds")]
#[required_permissions("MANAGE_GUILD")]
fn create(_ctx: &mut Context, msg: &Message, args: Args) -> Result<(), CommandError> {
    let mut args = args;
    // Unwrap is safe here because this command is only available in servers.
    let server_id = msg.guild_id.unwrap();
...
#[command]
#[description("Sets the values of the server if it's currently being edited (eg, after create or get). Start and end expressions are cron syntax but with an additionnal field on the left for seconds.")]
#[usage("<channel_id> <start exp> <end exp>")]
#[num_args(3)]
#[only_in("guilds")]
#[required_permissions("MANAGE_GUILD")]
fn set(_ctx: &mut Context, msg: &Message, args: Args) -> Result<(), CommandError> {
    let mut args = args;
...
#[command]
#[description("Adds roles to be @ed when the start message is sent.")]
#[usage("role_id [role_id, role_id, ...]")]
#[min_args(1)]
#[only_in("guilds")]
#[required_permissions("MANAGE_GUILD")]
fn add_roles(_ctx: &mut Context, msg: &Message, args: Args) -> Result<(), CommandError> {
    let mut args = args;
...
#[command]
#[description("Adds a game to the server configuration.")]
#[usage("<name> <emoji> <channel_id> [role_id, role_id, ...]")]
#[min_args(3)]
#[only_in("guilds")]
#[required_permissions("MANAGE_GUILD")]
fn add_game(_ctx: &mut Context, msg: &Message, args: Args) -> Result<(), CommandError> {
    let mut args = args;
...
#[command]
#[description("Adds the configuration made to the live configuration and saves it to disk. Ask an owner to rehash the configuration.")]
#[num_args(0)]
#[only_in("guilds")]
#[required_permissions("MANAGE_GUILD")]
fn commit(_ctx: &mut Context, msg: &Message, _args: Args) -> Result<(), CommandError> {
    // Unwrap is safe here because this command is only available in servers.
    let server_id = msg.guild_id.unwrap();
    let incomplete_servers = INCOMPLETE_SERVERS
        .read()
